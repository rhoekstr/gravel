...
contracted. Degree-2 contraction now optionally records the collapsed polyline. Additive and opt-in —
no change to existing behavior or the public ABI.
...
- **`SimplificationConfig.emit_geometry`** (default `false`). When set, `simplify_graph` /
  `contract_degree2` record each edge's coordinate chain — the real shape for merged degree-2 chains,
  a 2-point segment for kept edges — in `SimplificationResult.edge_geometry`. Honored for the
  filter + degree-2 pipeline; left empty if CH-level pruning also runs (it would misalign the edge
  set). Requires a graph with coordinates.
