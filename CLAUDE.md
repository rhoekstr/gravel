...
> Gravel is **published** on PyPI (`gravel-fragility`; current **2.6.0**). Changes here ship to real
...
| `datasets/` | dataset loaders + catalog: `osm_graph` (from `geo/`), `tiger_loader` (from `us/`), the hazard-source catalog (`dataset_catalog`) |
